...
public class SftpUtils {
    ...
    public SftpUtils(String defaultMethod) {

//        this.host = "10.117.196.22";
//        this.username = "absweb";
//        this.password = "Vtj8BXcwx5FHjqM";

//        this.host = "10.162.207.154";
//        this.username = "absweb";
//        this.password = "h3BnGLxep3baUZj7XL";
    }
    ...
    public void connect() {
        try {
            ...
            if (null == sftp) {
                throw new Exception("sftp 获取时为 null");
            }
        } catch (Exception e) {
            e.printStackTrace();
            logger.error("获取sftp连接时出错,{}",e);
        }
    }/**
     * 获取输出的out put stream
     * @param finalPath
     ...
     * @throws Exception
     */
    public OutputStream getUpLoadStream(String finalPath,String name) throws Exception{
...
//            disconnect();
    ...
    public byte[] downLoad(String remotePath, String remoteFile) throws Exception{
        try {
            logger.info("remotePath:{},remoteFile:{}",remotePath,remoteFile);
            if (sftp == null) connect();
            sftp.cd(remotePath);
     ...
     * 上传 不关闭ftp连接
     */
    public void uploadFileUnCloseFtp(String remotePath, String fileName, InputStream input) throws IOException, Exception {
        try {
            if (sftp == null) {
                connect();
            }
     ...
     * 上传 返回流
     */
    public OutputStream uploadWithOutput(String remotePath, String fileName) throws Exception {
        try {
            if (sftp == null) {
                connect();
            }
            createDir(remotePath);
            mkDir(remotePath);
            return sftp.put(remotePath+"/"+fileName);
        } catch (Exception e) {
            logger.error("获取文件上传流异常！", e);
            throw new Exception("获取文件上传流异常:" + e.getMessage());
        }
    }
